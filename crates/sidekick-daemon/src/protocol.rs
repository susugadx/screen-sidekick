...
use screen_sidekick_codex_client::{schema_hash, CodexClientError, CodexEvent, StartTurnRequest};
...
async fn handle_message_send(
    ...
    let outcome = match timeout(
        state.codex_start_timeout,
        state.codex.start_turn(codex_request),
    )
    .await
    {
